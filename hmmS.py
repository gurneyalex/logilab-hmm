...
This module is an heritage of the module HMM and uses numeric python multyarrays to improve performance and reduce memory usage """
...
class HMMS(CLASS):
    ...
    Methods are provided for computing the most probable state transitions leading to
    a sequence of observations, as well as supervised and unsupervised
    training of the model knowing states and generation of sequences of observations
    (simulation).
    ...
    def _learn_A(self, states):
        """Train the transition probability matrix according to one sequence of states"""
        T = len(states)
        N = self.N
        self.A = zeros((N,N))
        for k in range(T-1):
    ...
    def _multiple_learn_A(self, setStates):
        """Train the transition probability matrix according to a multiple states
        sequences"""
        hmm = HMMS(self.omega_X, [1])
        N = self.N
    ...
    def _baumWelch( self, obsIndices, states=None, maxiter=1000, impr=1 ):
        """Uses Baum-Welch algorithm to learn the probabilities knowing the states
        ...
        for iter in xrange( 1, maxiter + 1 ):
            ...
            ksi = self.Ksi( self.A, Bo, alpha, beta )
            gamma = self._gamma( alpha, beta, scale_factors )
            B_bar, pi_bar = self._final_step( gamma, ksi, obsIndices )
    ...
    def ensemble_averaging(self, setObservations, setStates, weighting_factor="unit", maxiter=1000, impr=1):
        """Uses ensemble averaging method to learn the probabilities on multiple
        observations sequences and states sequences"""
        N = self.N
        W = 0
        hmmk = HMMS(self.omega_X, self.omega_O, self.A, self.B, self.pi)
        A_bar = zeros( (N, N))
        ...
        for k, obs in enumerate(setObservations):
            ...
            state = setStates[k]
            nit, learningCurve = hmmk._baumWelch(obsIndices, state, maxiter, impr)
